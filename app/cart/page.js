    ...
    setCartItems(prevCart => {
      return prevCart.map((item, idx) => {
        if (idx === index) {
          ...
          const unitPrice = item.unitPrice || (item.price / item.quantity) || item.price;
          return {
            ...item,
  ...
  return (
    <div className='pt-20 px-5 text-white'>
      <ToastContainer
        ...
        transition={Bounce}
      />
      <h2 className="text-xl mb-4 font-semibold font-sans border-b border-gray-400/50 text-amber-100">Cart Items</h2>
      {loading ? (
        ...
        <>
          {cartItems.map((item, idx) => (
            <div key={idx} className="mb-2 border-b border-amber-200/50 pb-2 flex items-center justify-between">
              <div className='flex gap-4 px-2'>
                ...
                <span>
                  ...
                  Qnt: {item.quantity}
                  <br />
                  Price: {item.price}/- <span className="text-xs text-gray-300">(₹{item.unitPrice || (item.price / item.quantity) || item.price} each)</span>
                </span>
              </div>
      ...
      >My Placed <span className='text-amber-300'>O</span>rders</Link>
    </div>
  )
}
